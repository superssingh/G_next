...
const Footer = () => {
  return (
    <div className="flex flex-col w-full h-fit bottom-0 transition-all duration-700 p-4 bg-black bg-opacity-50 text-gray-400 hover:bg-opacity-70">
          ...
          {TagName.footermenus.map((m) => (
            <Link key={m.name} href={m.path}>
              <a
                tabIndex={1}
                data-text={m.name}
                className="underLine relative w-fit text-sm py-1 mx-1 transition-all duration-300 hover:text-white"
              >
                {m.name}
              </a>
            </Link>
